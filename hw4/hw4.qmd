...
chartevents_tble <- tbl(con_bq, "chartevents") |>
  ...
  # Select relevant columns: subject_id, itemid (chart event), storetime, and value (measurement)
  ...
  # Group by subject_id and itemid to handle cases where there are multiple measurements at the same time
  group_by(subject_id, itemid) |>
  
  # Keep only the first measurement for each subject and chart event (earliest storetime)
  filter(storetime == min(storetime)) |>
  
  # If multiple measurements exist at the first storetime, take the average of `value`
  summarise(value = mean(value, na.rm = TRUE), .groups = "drop") |>
  
  # Pivot wider to turn itemid into columns and each column will contain the corresponding value
...
```{r}
# #create our icu_cohort, start with icustays_tble
#  mimic_icu_cohort <- icustays_tble |>
#   
#   # Merge in the admissions and patients tables
#   inner_join(admissions_tble, by = c("subject_id", "hadm_id")) |>
#   inner_join(patients_tble, by = c("subject_id")) |>
#   
#   # Keep adults only
...
#   inner_join(labevents_tble, by = c("subject_id")) |>
#   inner_join(chartevents_tble, by = c("subject_id")) |>
#   
#   # Step (v) Collect the tibble (bringing data into R environment)
#   collect() |>
#   # Step (vi) Sort by subject_id, hadm_id, and stay_id
#   arrange(subject_id, hadm_id, stay_id) |>
#   
#   # Print all columns
#   print(width = Inf)
                ...
                box(title = "Enter Patient ID",
                    ...
                    selectizeInput("patient_id",
                                ...
                                options = list(
                                  ...
                                  maxItems = 1, #Limit to only 1 selection
                                  selectOnTab = TRUE #Allow selecting tab key
                                              )
                                )
                ),
                box(title = "Patient ADT & ICU Stay Info",
                    width = 8, solidHeader = TRUE, status = "primary",
                    DTOutput("patient_info_table"))
              ))
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # Tab 2: Display Patient Information from BigQuery
  output$patient_info_table <- renderDT({
    req(input$patient_id)  # Ensure the patient_id is selected
    
    # Create BigQuery client to query the database
    # Set up BigQuery authentication (make sure to authenticate with your Google Cloud credentials)
    bq_project <- "biostat-203b-2025-winter"
    bq_dataset <- "mimiciv_3_1"
    bq_table <- "mimic_icu_cohort"  # Make sure to adjust to your actual table name
    
    # Construct the SQL query to get the patient's ADT and ICU stay information
    query <- sprintf("SELECT * FROM `%s.%s.%s` WHERE subject_id = '%s'",
                 bq_project, bq_dataset, bq_table, input$patient_id)
    
    # Run the query and fetch the data
    patient_data <- bq_table_download(bq_project, bq_dataset, bq_table, query)
    ...
    tabItems(
      ...
      tabItem(tabName = "patient_info",
        fluidRow(
          ...
          box(title = "Choose Plot Type",
              width = 4, solidHeader = TRUE, status = "primary",
              selectInput("plot_type",
                          "Choose Plot Type",
                          choices = c("ADT Plot" = "ADT", 
                                      "ICU Plot" = "ICU"))
          ),
          box(title = "Patient ADT & ICU Stay Info",
              width = 8, solidHeader = TRUE, status = "primary",
...
server <- function(input, output, session) {
  ...
  patient_info <- reactive({
    ...
    # Extract patient demographics (assuming columns: gender, age, race, insurance)
  ...
  # Get top 3 diagnoses (for subtitle)
  top_diagnoses <- reactive({
    req(input$patient_id)
    patient_data <- patient_data_filtered()
    # Assuming 'diagnoses' is a list of diagnosis codes or labels
    diagnoses <- patient_data %>%
      count(diagnosis) %>%
      top_n(3, n) %>%
      arrange(desc(n)) %>%
      pull(diagnosis)
  ...
  output$patient_plot <- renderPlot({
    req(input$plot_type)  # Ensure a plot type is selected
    ...
    if (input$plot_type == "ADT") {
      ...
      geom_segment(aes(xend = dischtime, yend = admission_type, color = admission_location, size = 4)) +
      geom_point(aes(x = admittime, y = admission_type, shape = "ADT"), size = 4) +
      geom_point(aes(x = dischtime, y = admission_type, shape = "ADT"), size = 4) +
      scale_color_manual(values = c("ICU" = "red", "CCU" = "blue", "Other" = "gray")) +
      scale_size_continuous(range = c(1, 3)) +
      scale_shape_manual(values = c("ADT" = 16)) +  # Circle shape for ADT events
      labs(title = paste("ADT Plot for Patient", input$patient_id),
           x = "Calendar Time", y = "Event Type", color = "Care Unit", size = "ICU/CCU Size") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
      
    } else if (input$plot_type == "ICU") {
      ggplot(patient_data, aes(x = intime, y = outtime)) +
        geom_segment(aes(xend = outtime, yend = outtime), 
                     color = "green", size = 2) +
        geom_point(aes(x = intime, y = outtime), 
                   color = "orange", size = 4) +
        labs(title = paste("ICU Stay Plot for Patient", input$patient_id), 
             x = "ICU Start Time", y = "ICU End Time") +
        theme_minimal()
    }
  })
}
