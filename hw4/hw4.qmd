...
```{r}
#There are 4 million rows, this is too many
 mimic_icu_cohort <- icustays_tble |>
  
  # Step (ii) Merge in the admissions and patients tables
  inner_join(admissions_tble, by = c("subject_id", "hadm_id")) |>
  inner_join(patients_tble, by = c("subject_id")) |>
  
  #select(-hadm_id_y) |> #remove duplicate column
  #rename(hadm_id = hadm_id_x) |> #rename hadm_id column
  
  # Step (iii) Keep adults only (age at ICU intime >= 18)
  ...
  # Print all columns
  print(width = Inf)
  #print(unique(mimic_icu_cohort$race))
  ...
  mutate(mimic_icu_cohort, 
    ...
    # Step (ii) Collapse the levels of race into specified categories
     race = fct_collapse(race,
                    ASIAN = c("ASIAN - SOUTH EAST ASIAN", "ASIAN", "ASIAN - CHINESE", 
                              "ASIAN - KOREAN", "ASIAN - ASIAN INDIAN"),
                    BLACK = c("BLACK/AFRICAN AMERICAN", "BLACK/CAPE VERDEAN", "BLACK/AFRICAN", 
                              "BLACK/CARIBBEAN ISLAND"),
                    HISPANIC = c("HISPANIC/LATINO - SALVADORAN", "HISPANIC/LATINO - PUERTO RICAN", 
                    ...
                    WHITE = c("WHITE", "WHITE - RUSSIAN", "WHITE - OTHER EUROPEAN", "WHITE - BRAZILIAN", 
                              "WHITE - EASTERN EUROPEAN"),
                    Other = c("OTHER", "UNKNOWN", "UNABLE TO OBTAIN", "PATIENT DECLINED TO ANSWER", 
...
library(DT)
library(gtsummary)
library(bigrquery)  # Assuming you're using BigQuery to fetch data
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "summary",
              fluidRow(
                box(title = "Numerical Summaries", width = 6, solidHeader = TRUE, status = "primary", 
                    DTOutput("num_summary")),
                box(title = "Graphical Summaries", width = 6, solidHeader = TRUE, status = "primary", 
      ...
      tabItem(tabName = "patient_info",
              fluidRow(
                box(title = "Select Patient", width = 4, solidHeader = TRUE, status = "primary",
                    selectInput("patient_id", "Choose Patient ID", choices = unique(mimic_icu_cohort$subject_id))
                ),
                box(title = "Patient ADT & ICU Stay Info", width = 8, solidHeader = TRUE, status = "primary",
...
server <- function(input, output, session) {
  ...
  # Tab 1: Graphical Summaries
  output$graphical_summary <- renderPlot({
    # Example graphical summary for age distribution
    ggplot(mimic_icu_cohort, aes(x = anchor_age)) +
      geom_histogram(binwidth = 5, fill = "blue", color = "black") +
      theme_minimal() +
      labs(title = "Age Distribution in ICU Cohort", x = "Age", y = "Frequency")
  })
  ...
  output$patient_info_table <- renderDT({
    ...
    bq_table <- "your_table_name"  # Make sure to adjust to your actual table name
    
    # Construct the SQL query to get the patient's ADT and ICU stay information
    query <- sprintf("SELECT * FROM `%s.%s.%s` WHERE subject_id = %d",
