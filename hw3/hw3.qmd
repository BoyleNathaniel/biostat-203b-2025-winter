...
write _parquet (labevents, "labevent s _pq")
ds_par <- open_dataset("l abevents_pq") %>%
  select(subject_id, itemid, char ttime, valuenum) %>% 
  filter(itemid %in% c(50912,50971,50983,50902,50882, 
                       51221, 51301, 50931)) %>% 
  arrange(subject_id, charttime, itemid) 
labevents_pq <- ds_par %>% collect() 
print(format(object.size(labevent s_pq), units = 'auto'))
# display the number of rows
cat("Number of rows:" , nrow(labevents_pq), "\n") 

# Display the first 10 rows of the dataframe head
(labevents_pq, 10) 
```
...
lab_data <- patient_labevents %>% 
  select(subj ect_id, charttime, itemid) %>% 
  mutate(event_type = "Lab", event_time = charttime) %>% 
  left_join(pat ient_pr ocedures, by= "itemid") 
# Prepare the procedure data 
procedure_data <- pati ent_procedur es %>% 
  select(subj ect_id, procedure_time, icd9 code) %>% 
  mutate(event_type = "Procedure", event time procedure_time) 
# Prepare the diagnoses data
diagnosis_data <- patient _diagnoses %>% 
  select(subject_id, chartdate, icd9_code) %>% 
  mutate(event_type "Diagnosis", event_time = chartdate) 
# Combine all data
combined_data <- bind_rows(adt_data, lab_data, procedur e_data, diagnosis_data) 
...
ggplot(combined_data, aes(x = event_time, 
                          ...
                          color= care_uni t, 
                          size= is_icu)) + 
  geom_segment(aes(xend = event_time, yend = event _type)) + 
...
# we have already filtered the data with bash in Q1 and in hw2 
...
write_parquet (labevents, "labevents_pq") %>%
  ...
  select(subj ect_id, i t emid, st or etime, valuenum) %>% 
  filter(i temid %in% c(50912,50971,50983,50902,50882, 
                        51221, 51301, 50931)) %>% 
  arrange(subject_id, storetime, itemid) labevents_pq <- ds_par %>% 
  collect() 
...
labevents_filtered <- labevents_pq %>% 
  filter(itemid %in% item_ids) %>% 
  left_join(lab_measur ements, by = "itemid") 
# Convert storetime and intime to date-time objects 
labevents_filtered <- labevents_filtered %>% 
  mutate(storetime = as_datetime(st oretime)) 
icust ays_tble <- icust ays_tble %>% 
  mutate(intime = as_datetime(intime)) 
# Join the labevents with icustays to identify the last measurement 
# before ICU stay
final_data <- labevents_filtered %>% inner_join(icustays_tble, by = "subject_id") %>% filter(storetime <= intime) %>% group_by(subject_id, itemid) %>% 
  filter(storet ime == max(storetime)) %>%
  ungroup() 
...
final_labevents <- final_data %>% 
  select(subj ect_id, st ay_id, i temid, valuenum) %>% 
...
final_data_clean <- final_data %>% 
  group_by(subj ect_id, stay_id, itemid) %>% 
  summarize(valuenum = mean(valuenum, na.rm =TRUE), 
            .groups = "drop")

final_labevents <- final_data_clean %>% 
  select(subj ect_id, st ay_id, itemid, valuenum) %>% 
  pivot_wider(names_from = itemid, values_from = valuenum, 
              names_prefix = "lab_") 
...
final_labevents <- final_labevent s %>% 
  rename_with(- purrr : :map_chr(.x, f uncti on(item) { 
    ...
    if (is.na(label )) pasteO("Unknown_", item) else label})) 
# Check the result 
