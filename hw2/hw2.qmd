...
Yes. Read_csv and fread is able to distinguesh data and time formats automatically.
read.csv treats these same columns as "characters." 
Read_csv treates certain columns as "numeric" while read.csv and fread treates them as 
...
read.csv uses almost 3 times the memeory as read_csv and nearly 4 times the
memory of fread.
...
Yes the run time is slightly after after injesting (about 0.8 seconds faster).

How much memory does the result tibble use?

The memory taken is now 59.10 MB compared to the orignial 70.02 MB  
...
```{r}
#check values of read_csv directly
#head(df_read_csv)
#check values of hospital_expire_flag
#unique(df_read_csv$hospital_expire_flag)
#unique(df_read_csv$marital_status)
